...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.agentesEducativo.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.agentes-educativos.update", [$agentesEducativo->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nombre">{{ trans('cruds.agentesEducativo.fields.nombre') }}</label>
                <input class="form-control {{ $errors->has('nombre') ? 'is-invalid' : '' }}" type="text" name="nombre" id="nombre" value="{{ old('nombre', $agentesEducativo->nombre) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('apellido') ? 'is-invalid' : '' }}" type="text" name="apellido" id="apellido" value="{{ old('apellido', $agentesEducativo->apellido) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('documento') ? 'is-invalid' : '' }}" type="text" name="documento" id="documento" value="{{ old('documento', $agentesEducativo->documento) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('departamento') ? 'is-invalid' : '' }}" name="departamento_id" id="departamento_id">
                    @foreach($departamentos as $id => $departamento)
                        <option value="{{ $id }}" {{ (old('departamento_id') ? old('departamento_id') : $agentesEducativo->departamento->id ?? '') == $id ? 'selected' : '' }}>{{ $departamento }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('municipio') ? 'is-invalid' : '' }}" name="municipio_id" id="municipio_id">
                    @foreach($municipios as $id => $municipio)
                        <option value="{{ $id }}" {{ (old('municipio_id') ? old('municipio_id') : $agentesEducativo->municipio->id ?? '') == $id ? 'selected' : '' }}>{{ $municipio }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('telefono_celular') ? 'is-invalid' : '' }}" type="text" name="telefono_celular" id="telefono_celular" value="{{ old('telefono_celular', $agentesEducativo->telefono_celular) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('institucion') ? 'is-invalid' : '' }}" name="institucion" id="institucion">
                    ...
                    @foreach(App\AgentesEducativo::INSTITUCION_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('institucion', $agentesEducativo->institucion) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('rol') ? 'is-invalid' : '' }}" name="rol" id="rol">
                    ...
                    @foreach(App\AgentesEducativo::ROL_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('rol', $agentesEducativo->rol) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('rol'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('modalidad') ? 'is-invalid' : '' }}" name="modalidad" id="modalidad">
                    ...
                    @foreach(App\AgentesEducativo::MODALIDAD_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('modalidad', $agentesEducativo->modalidad) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('conectividad') ? 'is-invalid' : '' }}" name="conectividad" id="conectividad">
                    ...
                    @foreach(App\AgentesEducativo::CONECTIVIDAD_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('conectividad', $agentesEducativo->conectividad) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('terminos') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="terminos" id="terminos" value="1" {{ $agentesEducativo->terminos || old('terminos', 0) === 1 ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('privacidad') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="privacidad" id="privacidad" value="1" {{ $agentesEducativo->privacidad || old('privacidad', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ip') ? 'is-invalid' : '' }}" type="text" name="ip" id="ip" value="{{ old('ip', $agentesEducativo->ip) }}">
                @if($errors->has('ip'))
