...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.ciudade.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.ciudades.update", [$ciudade->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="name">{{ trans('cruds.ciudade.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $ciudade->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" type="text" name="code" id="code" value="{{ old('code', $ciudade->code) }}">
                @if($errors->has('code'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('departamento') ? 'is-invalid' : '' }}" name="departamento_id" id="departamento_id">
                    @foreach($departamentos as $id => $departamento)
                        <option value="{{ $id }}" {{ (old('departamento_id') ? old('departamento_id') : $ciudade->departamento->id ?? '') == $id ? 'selected' : '' }}>{{ $departamento }}</option>
                    @endforeach
                </select>
